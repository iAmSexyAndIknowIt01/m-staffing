...
export default function JobDetailView({ 
  ...
  return (
    <div className="max-w-3xl mx-auto pb-12 pt-2 px-3 sm:px-6">
      {/* 1. Header */}
      ...
      <div className="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden">
        ...
        {/* Header Section */}
        <div className="p-6 sm:p-8 border-b border-slate-100 flex items-start gap-5">
          <div onClick={(e) => handleCompanyClick(e, job.mt_company)} className="w-16 h-16 rounded-2xl bg-indigo-50 border border-indigo-100 flex items-center justify-center cursor-pointer transition-transform hover:scale-105">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 bg-slate-50/50">
          {[
            ...
            <div key={idx} className={`p-5 flex items-center gap-4 ${idx !== 2 ? 'border-b sm:border-b-0 sm:border-r border-slate-100' : ''}`}>
        ...
        {/* Content Section */}
        <div className="p-6 sm:p-10 space-y-10 border-t border-slate-100 bg-slate-50/30">
          <section>
            <h3 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
              <span className="w-1.5 h-6 bg-indigo-500 rounded-full"></span> Ажлын үүрэг
            </h3>
            <p className="text-[15px] text-slate-600 leading-relaxed whitespace-pre-line pl-4">{job.description}</p>
          </section>
          
          <section>
            <h3 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
              <span className="w-1.5 h-6 bg-emerald-500 rounded-full"></span> Тавигдах шаардлага
            </h3>
            <p className="text-[15px] text-slate-600 leading-relaxed whitespace-pre-line pl-4">{job.requirements}</p>
          </section>
        </div>
      </div>
    </div>
