...
export async function GET(request: Request) {
  try {
    ...
    // 🌟 ️ЭНД 'description' ТАЛБАРЫГ НЭМЖ ТАТСАН
    ...
    // Анкет илгээсэн ажлын байрнуудын мэдээллийг (Түүх хэсэгт харуулахын тулд) тусад нь татна
    // 🌟 ️ЭНД БАС 'description' ТАЛБАРЫГ НЭМЖ ТАТСАН
    let relatedJobs: any[] = [];
    if (appliedJobIds.length > 0) {
    ...
    const finalData = {
      ...
      // 1. Санал болгож буй ажлууд (Тайбарыг фронт руу дамжуулав)
      recommendedJobs: (openJobsResponse.data || []).map((job: any) => {
        ...
        return {
          ...
          description: job.description || "Ажлын тайлбар байхгүй байна." // 🌟 Фронтод зориулав
        };
      }),

      // 2. Илгээсэн анкет (Түүхэн дээр дарахад мөн тайлбарыг харуулж болно)
      recentApplications: (recentApplicationsResponse.data || []).map((app: any) => {
        ...
        return {
          ...
          description: correspondingJob?.description || "Ажлын тайлбар байхгүй байна." // 🌟 Фронтод зориулав
        };
      })
    };
