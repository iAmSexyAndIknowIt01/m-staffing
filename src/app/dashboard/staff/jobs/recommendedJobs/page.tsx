...
export default function RecommendedJobsPage() {
  ...
  // РЕАЛ-ТАЙМ ХАЙХ, АНГИЛЛААР ШҮҮХ ЛОГИК
  ...
  // ДАШБОРДООС БҮХ ИРЖ БУЙ АНГИЛЛУУДЫГ СОРТЛОЖ АВАХ (Шүүлтүүрийн select-д харуулах)
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen py-24 w-full">
        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-indigo-600"></div>
        <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-4">Ажлын байрууд уншиж байна...</p>
      </div>
    )
  }
  ...
  return (
    <div className="animate-fade-in space-y-6 max-w-5xl mx-auto p-4 md:p-8">
      ...
      {/* ХАЙЛТ БОЛОН АНГИЛЛААР ШҮҮХ ХЭСЭГ */}
      {jobs.length > 0 && (
      ...
      {jobs.length === 0 ? (
        <div className="bg-white border border-gray-100 rounded-4xl p-12 text-center text-gray-400">
          Одоогоор танд тохирох нээлттэй ажлын байр олдсонгүй.
        </div>
      ) : filteredJobs.length === 0 ? (
        <div className="bg-white border border-gray-100 rounded-4xl p-12 text-center text-gray-500 text-sm font-medium">
          😞 Хайлтын нөхцөлд тохирох ажлын байр олдсонгүй.
        </div>
      ) : (
